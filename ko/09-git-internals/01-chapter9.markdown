...
위에 보는 것과 같이 데이터 전송을 마치고 나면 `master` 브랜치를 확인해야 한다. 이 시점에서 이미 `ca82a6` 커밋 개체가 `info/refs`에서 발견할 수 있었기에 우리는 다음 과정으로 Fetch 과정을 진행할 수 있다:
...
개체는 서버에 일반적인 저장소의 모습으로 위치하고 있기 때문에 HTTP GET 요청으로 어렵지 않게 가져올 수 있다. 이렇게 서버로부터 얻어온 개체를 zlib로 압축을 풀고 header를 떼어내고 나면 다음과 같은 모습일 것이다:
...
그 다음 두 개의 개체를 더 내려받아야 한다. `cfda3b` 개체는 방금 내려받은 커밋 개체의 Tree 개체이고, `085bb3` 개체는 부모 커밋 개체이다:
...
위와 같이 커밋 개체는 내려받았다. 하지만 Tree 개체를 내려받으려고 하면:
...
이런! 존재하지 않는다는 404 메시지를 보면 해당 Tree 개체는 서버에 일반적인 개체처럼 저장되어 있지 않은가보다. 이런 상황이 벌어질 이유가 좀 있다. 해당 개체가 다른 저장소에 있을 경우도 있고 이 저장소의 Packfile 속에 있을 수도 있다. 우선 Git은 다른 저장소 목록을 통해 찾아본다:
...
이렇게 비어있는 다른 저장소 목록 데이터를 받게되면 Git은 Packfile에서 해당 개체를 검색한다. 이런 방식은 프로젝트를 Fork 하는 경우 디스크 공간을 효율적으로 사용할 수 있게 해준다. 우선 서버로부터 받은 다른 저장소 목록이 비어있기 때문에 개체는 확실히 Packfile 속에 있을 것이다. 서버에서 어떤 Packfile들이 있는지 살펴보려면 `objects/info/packs` 파일이 필요하다. 이 파일 또한 `update-server-info` 명령에 의해 작성된다.
...
이제 Packfile의 인덱스를 가져왔기 때문에 찾고자 하는 개체의 위치를 파악할 수 있다. 해당 Packfile을 내려받도록 한다:
...
Tree 개체를 얻어오고 나면 다시 커밋에 대한 데이터를 얻어오는 과정을 계속 할 수 있다. 아마도 방금 내려받은 Packfile 속에 모든 커밋 데이터를 포함하고 있기 때문에 서버로 다시 데이터 전송 요청을 보내지 않아도 된다. Git은 처음에 확인한 대로 HEAD가 가리키고 있는 `master` 브랜치에 대한 소스코드를 복원해놓을 것이다.
...
이 시점에서 `fetch-pack`은 어떤 개체를 이미 갖고 있는지 살펴보고 서버로부터 내려받아야 개체에 대해서 "want"를 앞에 붙이고 SHA 값을 전송한다. 이미 갖고 있는 개체에 대해서는 "have"를 앞에 붙이고 SHA 값을 전송한다. 마지막 줄에 "done"이라고 적어서 보내면, 서버의 `upload-pack`이 Packfile의 전송을 시작한다:
